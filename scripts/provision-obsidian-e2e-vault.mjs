...
const DEFAULT_VAULT_PREFIX = "notetweet";
const PLUGIN_ID = "notetweet";

// Expression evaluated via `obsidian eval` to confirm the NoteTweet plugin
...
export const NOTETWEET_READY_EVAL = `Boolean(app.plugins.plugins[${JSON.stringify(PLUGIN_ID)}])`;

// A valid, default NoteTweet settings document. Mirrors DEFAULT_SETTINGS in
// src/settings.ts so a freshly provisioned vault loads with clean plugin
// state. Keep in sync with that file.
export const DEFAULT_NOTETWEET_DATA = {
	postTweetTag: "",
...
Options:
  --vault <name>        Vault name to provision. Defaults to notetweet-<worktree>.
  --root <path>         Directory that contains provisioned vaults. Defaults to .obsidian-e2e-vaults.
  --worktree <path>     NoteTweet worktree to link plugin files from. Defaults to cwd.
  --data <path>         Optional NoteTweet data.json seed to copy on first provision.
  --force               Recreate plugin symlinks if they already exist.
  --print-env           Print NOTETWEET_E2E_VAULT exports after provisioning.
...
async function assertRequiredPluginFiles(worktreePath) {
	...
	if (missing.length > 0) {
		throw new Error(
			[
				`Cannot provision NoteTweet in ${worktreePath}; missing ${missing.join(", ")}.`,
...
export async function provisionVault(options) {
	...
	await writeJsonIfMissing(path.join(obsidianPath, "workspace.json"), {
		main: { id: "notetweet-e2e", type: "split", children: [] },
		left: { id: "notetweet-e2e-left", type: "split", children: [] },
		right: { id: "notetweet-e2e-right", type: "split", children: [] },
	});
	...
	if (options.dataPath && !(await pathExists(pluginDataPath))) {
		...
		await writeJsonIfMissing(pluginDataPath, DEFAULT_NOTETWEET_DATA);
	}
...
export function toShellExports(result) {
	return [
		`export NOTETWEET_E2E_VAULT=${shellQuote(result.vaultName)}`,
		`export NOTETWEET_E2E_VAULT_PATH=${shellQuote(result.vaultPath)}`,
	].join("\n");
}
...
async function main() {
	...
	if (options.json) {
		...
		status(`NoteTweet plugin: ${result.pluginPath}`);
