...
describe("start-obsidian-e2e-instance", () => {
	it("derives a private profile HOME under the profile root", () => {
		const options = resolveInstanceOptions(
			...
			"/tmp/notetweet-instance",
		);

		expect(
			options.obsidianHome.startsWith(
				path.join("/tmp/notetweet-instance", "profiles", "notetweet-worktree-a-"),
			),
		).toBe(true);
		expect(options.obsidianHome.endsWith("/home")).toBe(true);
		expect(options.vaultPath).toBe(
			path.join("/tmp/notetweet-instance", "vaults", "notetweet-worktree-a"),
		);
	});

	it("creates a private Obsidian profile registered to the provisioned vault", async () => {
		const cwd = await makeTempDir("notetweet-instance");
		const options = resolveInstanceOptions(
			parseArgs([
				"--vault",
				"notetweet-worktree-a",
		...
		expect(
			options.obsidianHome.startsWith(
				path.join(cwd, "profiles", "notetweet-worktree-a-"),
			),
		).toBe(true);
		expect(options.obsidianHome.endsWith("/home")).toBe(true);
		expect(vaults).toEqual([
			{
				open: true,
				path: path.join(cwd, "vaults", "notetweet-worktree-a"),
