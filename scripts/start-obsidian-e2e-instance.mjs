...
const DEFAULT_PROFILE_ROOT = "/tmp/notetweet-obsidian-e2e";
...
export const INSTANCE_MARKER_FILE = "notetweet-e2e-instance.json";
const DEFAULT_OBSIDIAN_APP = "Obsidian";
const DEFAULT_OBSIDIAN_BIN = "obsidian";
const READY_TIMEOUT_MS = 30_000;
const READY_INTERVAL_MS = 500;
const NOTETWEET_READY_MARKER = "=> true";

function printUsage() {
	console.log(`Usage: node scripts/start-obsidian-e2e-instance.mjs [options]

Options:
  --vault <name>        Vault/profile name. Defaults to notetweet-<worktree>.
  ...
  --profile-root <path> Directory for per-vault Obsidian HOME profiles. Defaults to /tmp/notetweet-obsidian-e2e.
...
async function linkHostKeychains(options) {
	...
	// `export HOME=$NOTETWEET_E2E_OBSIDIAN_HOME`), source and destination are the
...
export async function trustVaultAndVerifyNoteTweet(options) {
	...
	while (Date.now() < deadline) {
		try {
			const { stdout } = await execObsidian(options, [
				`vault=${options.vaultName}`,
				"eval",
				`code=${NOTETWEET_READY_EVAL}`,
			]);
			if (stdout.includes(NOTETWEET_READY_MARKER)) return true;
	...
	throw new Error(
		`NoteTweet did not become available in ${options.vaultName}. Last error: ${lastError}`,
	);
}

export async function reloadNoteTweet(options) {
...
export function toInstanceShellExports(result) {
	const lines = [
		...
		`export NOTETWEET_E2E_OBSIDIAN_HOME=${shellQuote(result.obsidianHome)}`,
	];
...
async function main() {
	...
	if (rawOptions.json) {
		...
		if (result.verifiedNoteTweet) status("NoteTweet plugin check: ok");
	}
