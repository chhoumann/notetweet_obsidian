...
const INSTANCE = "/tmp/notetweet-obsidian-e2e/notetweet-wt-abc123def456";
...
const PS_FIXTURE = [
	...
	`  101   100 /Applications/Obsidian.app/Contents/Frameworks/Obsidian Helper (GPU).app/Contents/MacOS/Obsidian Helper (GPU) --type=gpu-process --user-data-dir=/private/tmp/notetweet-obsidian-e2e/notetweet-wt-abc123def456/home/Library/Application Support/obsidian`,
	`  102   100 /Applications/Obsidian.app/Contents/Frameworks/Obsidian Helper (Renderer).app/Contents/MacOS/Obsidian Helper (Renderer) --type=renderer --user-data-dir=/private/tmp/notetweet-obsidian-e2e/notetweet-wt-abc123def456/home/Library/Application Support/obsidian`,
	...
	"  400     1 /Applications/Obsidian.app/Contents/MacOS/Obsidian --user-data-dir=/tmp/notetweet-obsidian-e2e/notetweet-other-111111111111/home/Library/Application Support/obsidian --password-store=basic",
].join("\n");
...
describe("commandMatchesInstance", () => {
	it("matches both the /tmp and /private/tmp forms of the instance path", () => {
		...
		expect(
			commandMatchesInstance(
				"x --user-data-dir=/private/tmp/notetweet-obsidian-e2e/notetweet-wt-abc123def456/home/x",
				INSTANCE,
			),
		).toBe(true);
	});
	...
	it("does not match the dev vault, quickadd, or a sibling worktree", () => {
		...
		expect(
			commandMatchesInstance(
				"--user-data-dir=/tmp/notetweet-obsidian-e2e/notetweet-other-111111111111/home/x",
				INSTANCE,
			),
		).toBe(false);
	});
...
describe("orphan detection", () => {
	...
	it("reads the registered vault paths", async () => {
		const root = await makeTempDir("notetweet-profiles");
		const instance = await seedInstance(
			root,
			"notetweet-a-aaaaaaaaaaaa",
			"/x/y",
		);
	...
	it("is orphaned only when every backing vault is gone", async () => {
		...
		const live = await seedInstance(
			root,
			"notetweet-live-bbbbbbbbbbbb",
			liveVault,
		);
		const gone = await seedInstance(
			root,
			"notetweet-gone-cccccccccccc",
			"/does/not/exist",
		);
	...
	it("prefers the worktree marker: worktree-gone reaps even with a live vault; worktree-alive spares even with a missing vault", async () => {
		...
		const spared = await seedInstance(
			root,
			"notetweet-m1-aaaaaaaaaaaa",
			"/vault/leaf/gone",
		);
		await fs.writeFile(
			path.join(spared, "notetweet-e2e-instance.json"),
		...
		const leaked = await seedInstance(
			root,
			"notetweet-m2-bbbbbbbbbbbb",
			liveWorktree,
		);
		await fs.writeFile(
			path.join(leaked, "notetweet-e2e-instance.json"),
	...
	it("stays conservative when the registration is unreadable", async () => {
		const root = await makeTempDir("notetweet-profiles");
		const bare = path.join(root, "notetweet-bare-dddddddddddd");
...
describe("reapOrphanedInstances", () => {
	...
	it("reaps only orphaned instances, skipping live ones, the exception, and non-instance dirs", async () => {
		...
		const current = await seedInstance(
			root,
			"notetweet-current-dddddddddddd",
			"/also/gone",
		);
	...
	it("continues reaping the rest of the scan when one orphan's teardown fails", async () => {
		...
		const bad = await seedInstance(
			root,
			"notetweet-bad-aaaaaaaaaaaa",
			"/gone/1",
		);
		const good = await seedInstance(
			root,
			"notetweet-good-bbbbbbbbbbbb",
			"/gone/2",
		);
		const removed: string[] = [];
		const logs: string[] = [];
