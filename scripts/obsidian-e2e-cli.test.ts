...
describe("obsidian-e2e-cli", () => {
	...
	it("uses -- to pass option-like Obsidian command arguments", () => {
		...
		expect(parsed.instanceArgs).toEqual(["--vault", "notetweet-worktree-a"]);
	...
	it("prefixes commands with the resolved isolated vault", () => {
		expect(
			obsidianCommandArgs({ vaultName: "notetweet-worktree-a" }, [
				"eval",
				"code=app.vault.getName()",
			]),
		).toEqual([
			"vault=notetweet-worktree-a",
			"eval",
			"code=app.vault.getName()",
		]);
	});
	...
	it("runs Obsidian CLI commands with the isolated HOME", () => {
		expect(obsidianEnv({ obsidianHome: "/tmp/notetweet/home" })).toMatchObject({
			HOME: "/tmp/notetweet/home",
		});
	});
});
